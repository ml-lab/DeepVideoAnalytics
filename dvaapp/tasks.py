...
def extract_frames(video_id):
    ...
    try:
        ...
        pass
    dv = Video.objects.get(id=video_id)
    v = wmodels.WVideo(dvideo=dv, temp_dir=TEMP_DIR)
    v.download()
    time.sleep(10) # otherwise ffprobe randomly fails
    v.get_metadata()
    ...
    for f in frames:
        ...
        df.bucket = dv.bucket
        df.save()
    perform_detection_video.apply_async(args=[dv.id, ], queue=Q_DETECTOR)
    finished = TEvent()
